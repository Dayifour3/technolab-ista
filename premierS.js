...
ipcMain.on("process-image", (event, imagePath, directeur, emplacement, nom)=>{
    const emplacement_fich = emplacement
    const directeur_nom = directeur
    const name = nom
    const workbook = new ExcelJS.Workbook();
    let worksheet = [];
    worksheet[0] = workbook.addWorksheet(`Resultat1 ${name}`);
    worksheet[1] = workbook.addWorksheet(`Resultat2 ${name}`);
    // const excelFilePath = path.join(__dirname, imagePath);



    async function appel(){
    for(let sek=0; sek<2; sek++){

    const workBook = XLSX.readFile(imagePath);
...
/*Fin Fonction Pour verifier si une matière est validé ou pas*/
        ...
        for(k=0;k<taille_du_fichier;k++){
            ...
            nom = nom_complet_tab[0];

            nom_complet_tab.shift();
            ...
            if(sek === 0){
                ...
                for(i=4;i<jsonData.length; i++){
                    if(countProperty(i)===11){
                        if(verifier_la_validite(i)!==""){
                                tab_de_matiere_non_valide.push(verifier_la_validite(i));
                        }
                ...
                for(i=new_debut;i<jsonData.length; i++){
                    if(countProperty(i)===11){
                        if(verifier_la_validite(i)!==""){
                                tab_de_matiere_non_valide.push(verifier_la_validite(i));
                        }
            ...
            if(i>=jsonData.length){
                worksheet[sek].getColumn(1).width = 3
                worksheet[sek].getColumn(2).width = 24
                worksheet[sek].getColumn(3).width = 15
                worksheet[sek].getColumn(4).width = 16
                worksheet[sek].getColumn(5).width = 28
                /* En-tête*/
                
                worksheet[sek].getRow(1).values = ["TECHNOLAB-ISTA/ANNEXE SOTUBA"]
                worksheet[sek].getRow(2).values = [`ANNEE SCOLAIRE ${annee_scolaire}`]
                worksheet[sek].getRow(3).values = ["RESULTAT DE LA DEUXIEME SESSION"]
                worksheet[sek].getRow(4).values = [`CLASSE : ${classe_filiere}`]
                
                worksheet[sek].getRow(6).values = ["N°", "PRENOM", "NOM", "Appreciation du jury", "UE à reprendre"]
                
                let insert_info_eleve = worksheet[sek].getRow(7+k);
                ...
                if (k+1 == taille_du_fichier){
                    ...
                    worksheet[sek].addImage(imageId1, {
                        tl: { col: 4, row: 1 },
                        br: { col: 5, row: 5 }
                    });
                    ...
                    worksheet[sek].getRow(7+ taille_du_fichier).values = [""]
                    worksheet[sek].getRow(7+taille_du_fichier+1).values = [""]
                    worksheet[sek].getRow(7+taille_du_fichier+2).values = ["","","", { formula: 'NOW()'}];
                    worksheet[sek].mergeCells(`${"D"+(7+taille_du_fichier+2)}:${"E"+(7+taille_du_fichier+2)}`)

                    worksheet[sek].getRow(7+taille_du_fichier+3).values = [""]
                    worksheet[sek].getRow(7+taille_du_fichier+4).values = ["","","","P/La Direction des Etudes et de la Formation/PO"]
                    worksheet[sek].mergeCells(`${"D"+(7+taille_du_fichier+4)}:${"E"+(7+taille_du_fichier+4)}`)

                    worksheet[sek].getRow(7+taille_du_fichier+5).values = ["","","","Responsable du Centre SOTUBA"]
                    worksheet[sek].mergeCells(`${"D"+(7+taille_du_fichier+5)}:${"E"+(7+taille_du_fichier+5)}`)

                    worksheet[sek].getRow(7+taille_du_fichier+6).values = [""]
                    worksheet[sek].getRow(7+taille_du_fichier+7).values = [""]

                    worksheet[sek].getRow(7+taille_du_fichier+8).values = ["","","",`${directeur_nom}`]
                    worksheet[sek].mergeCells(`${"D"+(7+taille_du_fichier+8)}:${"E"+(7+taille_du_fichier+8)}`)


                    worksheet[sek].getRow(7+taille_du_fichier+2).eachCell((cell)=>{
                    ...
                    worksheet[sek].getRow(7+taille_du_fichier+4).eachCell((cell)=>{
                        cell.numFmt = ',,',
                        cell.font = {
                            bold: true,
                            underline: true,
                    ...
                    worksheet[sek].getRow(7+taille_du_fichier+5).eachCell((cell)=>{
                        cell.numFmt = ',,',
                        cell.font = {
                            bold: true,
                            size: 10,
                    ...
                    worksheet[sek].getRow(7+taille_du_fichier+8).eachCell((cell)=>{
                        cell.numFmt = ',,',
                        cell.font = {
                            bold: true,
                            size: 10,
                ...
                for(let compte_ligne =1; compte_ligne<5; compte_ligne++){
                    worksheet[sek].getRow(compte_ligne).eachCell((cell)=>{
                        cell.font  = {
                            bold : true,
                            size : 14,
                                color : { theme : 1 },
                ...
                worksheet[sek].getRow(6).eachCell((cell)=>{
                    {
                        cell.numFmt = '#,##0',
                        cell.font = {
                        bold: true,
    ...
    console.log('Le fichier mon_fichier.xlsx a été généré avec succès !');
}
